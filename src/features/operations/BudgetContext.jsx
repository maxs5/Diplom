...
export function BudgetProvider({ children }) {
  ...
  const createBudget = (data) => {
    try {
      ...
      if (budgets.find(b => b.categoryId === categoryId && b.period === period)) {
        return { success: false, error: 'Бюджет для этой категории уже существует' };
      }
  ...
  const updateBudget = (id, updates) => {
    try {
      ...
      if (index !== -1) {
        allBudgets[index] = { ...allBudgets[index], ...updates, updatedAt: new Date().toISOString() };
        localStorage.setItem(STORAGE_KEYS.BUDGETS, JSON.stringify(allBudgets));
      }
  ...
  const getBudgetsWithStatus = () => {
    return budgets.map(budget => ({
      ...
      percentage: Math.round((getSpentByCategory(budget.categoryId, budget.period) / budget.limit) * 100),
      isExceeded: getSpentByCategory(budget.categoryId, budget.period) > budget.limit,
    }));
  };
