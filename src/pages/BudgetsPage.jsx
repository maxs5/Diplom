...
export function BudgetsPage() {
  ...
  const { createBudget, deleteBudget, getBudgetsWithStatus, loading: budgetsLoading } = useBudget();
        ...
        {budgetsWithStatus.length === 0 ? (
          <Card>
            <div className="empty-state">
              ...
              <p className="text-secondary">Создайте первый бюджет, чтобы контролировать расходы</p>
            </div>
          </Card>
        ) : (
            ...
            <Card key={budget.id} className="budget-card">
              <div className="budget-header">
                <div className="budget-info">
                  <span className="budget-icon">{getCategoryIcon(budget.categoryId)}</span>
                  <div className="budget-details">
                    <h3>{getCategoryName(budget.categoryId)}</h3>
                    <p className="text-secondary">{getPeriodLabel(budget.period)}</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
              ...
              <div className="budget-progress">
                ...
                <div className="progress-text">
                  ...
                  <span className="text-secondary">/ {formatCurrency(budget.limit)}</span>
                </div>
              </div>

              {budget.isExceeded && (
                <div className="warning-message">
                  ⚠️ Бюджет превышен на {formatCurrency(budget.spent - budget.limit)}
                </div>
              )}

              <div className="progress-percentage">
                <span className={`percentage ${budget.percentage > 100 ? 'exceeded' : ''}`}>
                  {budget.percentage}%
                </span>
              </div>
            </Card>
